...
const HOADONLD = () => {
    ...
    const fetchProducts = async (search) => {
        if (!search) { search = '' }
        const manufac = await axios.get(`http://localhost:3500/bill/install/search?type=${typeSearch}&search=${search}`)
                        ...
                        {manufactures && manufactures.length > 0
                            ...
                            manufactures[page - 1].map((item, index) => (
                                <tr key={index} className={Styles.rowTable}>
                                    ...
                                    <td>{item.SDT}</td>
                                    <td>{item.NgayTao}</td>
                                    <td>{item.TienDoHD == 0 ? (<span>Đợi lắp đặt</span>) : item.TienDoHD == 1 ? (<span>Đang tiến hành</span>) : (<span>Đã xong</span>)}</td>
