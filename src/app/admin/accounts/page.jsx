...
import Addmanufacture from '@/component/nhanvien/addmanufacture/Addmanufacture';
import Updatemanufacture from '@/component/nhanvien/updatemanufacture/updatemanufacture';
...
const Manufacturer = () => {
    ...
    const [typeSearch, setTypeSearch] = useState('HoVaTen')
    ...
    const fetchProducts = async (search) => {
        if (!search) { search = '' }
        const manufac = await axios.get(`http://localhost:3500/nhanvien/search?type=${typeSearch}&search=${search}`)
    ...
    const handleDeleteOne = async (item) => {
        ...
        try {
            const response = await axios.delete('http://localhost:3500/nhanvien', {
                data: { productIds: item.MaNV }
            });
        ...
        <div className={Styles.container}>
            ...
            <div className={Styles.actionbar}>
                <select className={Styles.dropdown} onChange={handleChangleType}>
                    <option value="HoVaTen">Tên Nhân Viên</option>
                ...
                <button className={`${Styles.btnAdd} ${display ? Styles.btnAddShow : ''}`} onClick={handleClickAdd}><FaPlus />Nhân viên mới </button>
